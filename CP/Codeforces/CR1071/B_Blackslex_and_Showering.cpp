...
void Solve()
{ 
  int n;cin>>n;vector<int>a(n);scan(a);
  int ans=0;
  for(int i=1;i<n;i++){
      ans+=abs(a[i]-a[i-1]);
  }
  ...
  for(int i=0;i<n;i++){
    if(i==0){
        anss=min(anss,abs(ans-abs(a[i]-a[i+1])));
    }else if(i==n-1){
        anss=min(anss,abs(ans-abs(a[i]-a[i-1])));
    }else{
        anss=min(anss,abs(abs(a[i+1]-a[i-1])+ans-(abs(a[i]-a[i-1])+abs(a[i]-a[i+1]))));
    }
  }
cout<<anss<<endl;
